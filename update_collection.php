...
if(isset($_POST["add_collection"])){
    ...
    $sql = "UPDATE collection set collection_name = '$collection_name',collection_artist ='$artist_name', collection_image ='$collection_image_name' where id = $id  ";
    $req = mysqli_query($con,$sql);
...
<body style="background-image:url(img/background.gif); background-repeat: no-repeat;background-size: cover;">
    ...
    <div class="cform">
        <div class="form-collection" style="height:450px ;">
            <div class="title">Update collection</div>
            
            <form action="" method="POST" enctype="multipart/form-data">
            <div class="input-container ic2">
              <input id="collectionname" class="input" type="text" placeholder="Collection name" name="collection_name" value="<?=$row["collection_name"]?>" />
            </div>
            <div class="input-container ic2">
              <input id="artistname" class="input" type="text" placeholder="Artist name" name ="artist_name" value="<?=$row["collection_artist"]?>"  />
            </div>
            <div class="input-container ic2" style="margin-top: 15px;">
                <input id="imagelink" class="input" type="file" placeholder="Image link"  name = "collection_image" value="<?=$row["collection_image"]?>" >
              </div>
            <a href="collection.php"><button type="submit" class="submit" name="add_collection">Update</button></a>
            </form>
        </div>
